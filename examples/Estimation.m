...
seizureDat = dir('status seizures/Seizure*');

for iSz = 2:13
    
    szName = seizureDat(iSz).name;
    load(['status seizures/' szName]);
    mkdir(['status seizures/estimates/' szName(1:11)])
    savePath = ['status seizures/estimates/' szName(1:11) '/'];
    addpath(genpath('../src/'));
    ...
    for iCh = 1:16
        ...
        savename = sprintf('%sCh%d.mat',savePath, iCh)
        ...
        for t=2:N_samples       % N_samples
            ...
            try
                ...
                fprintf('filter error saving Ch%d at %d%%', iCh,round(100*t/N_samples))
                save(savename,'xi_hat','P_diag','-v7.3');
                break
            end
